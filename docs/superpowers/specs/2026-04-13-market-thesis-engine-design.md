...
**Date:** 2026-04-13  
**Status:** Approved  
**Scope:** Strengthen finance-related capabilities via a `market` aggregation command set

---

## Goal

Add a `market` command set that aggregates data from multiple existing finance sites into a unified, structured analysis. Both manual CLI use and AI agent pipelines are first-class use cases.
...
```sh
market quote      --symbol X
market flow       --symbol X
market technicals --symbol X
market sentiment  --symbol X
market thesis     --symbol X
```
...
| insiderfinance | Optional; skipped when not logged in |

### Technicals Dimension
| Site | Change |
|---|---|
| barchart | Normalize technicals output |
| tradingview | Add `technicals` command |
...
| reddit | Add symbol search command |
...
3. Normalize unusual-whales and whalestream output
4. `market flow --symbol X` working end-to-end

### Phase 2 - Quote + Technicals
1. `src/sites/market/quote.mjs`
2. `src/sites/barchart/` technicals normalization
3. `src/sites/tradingview/technicals.mjs`
...
- `market thesis --symbol NVDA --quick --port 9223` returns a complete structured result in < 8s
- Each sub-command is independently callable
- Source failures degrade gracefully with clear `sources_skipped` metadata
- Output matches the unified schema
- Agents can consume `thesis.bias`, `thesis.confidence`, and `thesis.flags` directly
